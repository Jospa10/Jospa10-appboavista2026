  ...
  return (
    <div className={`bg-slate-900 p-5 rounded-xl border transition-all relative group ${isPast ? 'border-slate-800' : isCanceled ? 'border-red-900/30 opacity-75' : 'border-emerald-500/30 shadow-lg shadow-emerald-500/5'}`}>
      ...
      <div className="flex justify-between items-center mb-4 pr-8">
        <span className={`text-[9px] font-black px-2 py-0.5 rounded border uppercase tracking-widest ${getStatusColor()}`}>{match.status}</span>
      ...
      <div className="flex items-center justify-between gap-4 py-4">
        <div className="flex-1 flex flex-col items-center gap-2">
          <div className="w-12 h-12 bg-slate-950 rounded-xl flex items-center justify-center overflow-hidden border border-emerald-500/20">
            {teamLogo ? <img src={teamLogo} className="w-full h-full object-cover" /> : <span className="text-xl">⚽</span>}
          </div>
          <p className="text-[10px] font-bold truncate text-emerald-500 uppercase tracking-tighter">{teamName}</p>
        </div>
        <div className="flex items-center gap-3">
          {isPast ? <div className="flex gap-3 items-center text-3xl font-sport tracking-widest text-white"><span>{match.scoreHome ?? 0}</span><span className="text-slate-700 text-xl">-</span><span>{match.scoreAway ?? 0}</span></div> : <div className="bg-slate-950 px-4 py-1.5 rounded-lg border border-slate-800 text-[10px] font-black text-slate-500 tracking-widest">VS</div>}
        </div>
        <div className="flex-1 flex flex-col items-center gap-2">
          <div className="w-12 h-12 bg-slate-950 rounded-xl flex items-center justify-center overflow-hidden border border-slate-800">
            {match.opponentLogo ? <img src={match.opponentLogo} className="w-full h-full object-cover" /> : <span className="text-xl">🛡️</span>}
          </div>
      ...
      <div className="mt-5 flex gap-2">
        {isPast ? (
          <button onClick={() => onOpenTechnicalSheet?.(match)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-[0.2em] border border-slate-700">Ficha Técnica</button>
        ) : !isCanceled ? (
          <>
            <button className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 rounded-xl text-[10px] uppercase tracking-[0.2em]">Confirmar Presença</button>
            {isAdmin && <button onClick={() => onUpdateStatus?.(match.id, MatchStatus.FINISHED)} className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-4 py-2.5 rounded-xl text-[10px] uppercase tracking-[0.2em]">✓</button>}
          </>
        ) : <div className="flex-1 bg-slate-800/50 text-slate-600 text-center py-2 rounded-xl text-[9px] font-bold uppercase tracking-[0.2em]">Cancelada</div>}
      </div>
    </div>
  );
};
